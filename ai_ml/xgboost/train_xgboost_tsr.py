#!/usr/bin/env python
"""Train an XGBoost classifier for TSR key-frequency feature tables."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an XGBoost classifier for TSR features.")
    ...
    parser.add_argument(
        "--group_split_candidates",
        type=int,
        default=512,
        help="Candidate grouped partitions evaluated when using grouped splitting.",
    )
    ...
    parser.add_argument(
        ...
        help="Training-set-only imbalance handling. class_weight becomes XGBoost sample weights.",
    )
    ...
    parser.add_argument("--min_child_weight", type=float, default=1.0, help="Minimum child weight.")
    ...
    parser.add_argument("--gamma", type=float, default=0.0, help="Minimum loss reduction for a split.")
    ...
    parser.add_argument(
        ...
        default="cpu",
        choices=["cpu", "cuda"],
        help="Use cpu or cuda. CUDA requires a GPU node and GPU-enabled XGBoost.",
    )
...
def main() -> None:
    ...
    model = build_xgboost_model(args, len(class_names))
    start = time.time()
    fit_xgboost(
        model=model,
    ...
    save_artifact(model, paths["models"] / "xgboost_model.joblib")
    ...
    print(f"XGBoost training complete. Outputs saved to: {output_dir.resolve()}")


def build_xgboost_model(args: argparse.Namespace, num_classes: int):
    try:
...
def build_sample_weights(balance_method: str, y_train: np.ndarray):
    """Use inverse-frequency sample weights for XGBoost class_weight mode."""
...
def fit_xgboost(*, model, X_train, y_train, X_val, y_val, sample_weight):
    try:
        model.fit(
            X_train,
            y_train,
        ...
        if "cuda" in message or "gpu" in message:
            raise RuntimeError(
                "XGBoost failed while using CUDA. Submit the CPU sbatch or install a GPU-enabled XGBoost/RAPIDS stack."
            ) from exc
        raise
...
def save_training_history(model, paths: Dict[str, Path]) -> None:
    ...
    for dataset_name, metrics in results.items():
        ...
        for round_index in range(n_rounds):
            row = {"dataset": dataset_name, "round": round_index + 1}
...
def save_feature_importance(model, feature_names, paths: Dict[str, Path], top_n: int) -> None:
    ...
    for idx, feature_name in enumerate(feature_names):
        score = importance.get(f"f{idx}", 0.0)
        rows.append({"feature": feature_name, "gain_importance": float(score)})
    importance_df = pd.DataFrame(rows).sort_values("gain_importance", ascending=False)
    ...
    plt.barh(plot_df["feature"], plot_df["gain_importance"], color="#805AD5")

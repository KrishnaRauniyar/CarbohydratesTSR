#!/usr/bin/env python
"""Train a CatBoost classifier for TSR key-frequency feature tables."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a CatBoost classifier for TSR features.")
    ...
    parser.add_argument(
        ...
        help="Training-set-only imbalance handling. class_weight becomes CatBoost sample weights.",
    )
    ...
    parser.add_argument("--iterations", type=int, default=1000, help="Maximum boosting iterations.")
    ...
    parser.add_argument("--random_strength", type=float, default=1.0, help="Randomness in split scoring.")
    parser.add_argument("--bagging_temperature", type=float, default=1.0, help="Bayesian bootstrap temperature.")
    parser.add_argument("--early_stopping_rounds", type=int, default=50, help="Validation early stopping rounds.")
    parser.add_argument("--thread_count", type=int, default=-1, help="CPU threads.")
    parser.add_argument(
        "--task_type",
        default="CPU",
        choices=["CPU", "GPU"],
        help="CatBoost execution mode. GPU requires a GPU node.",
    )
    parser.add_argument("--devices", default="0", help="GPU device IDs for CatBoost GPU mode.")
    parser.add_argument("--verbose", type=int, default=100, help="CatBoost logging frequency.")
...
def main() -> None:
    ...
    model = build_catboost_model(args, len(class_names))
    start = time.time()
    fit_catboost(
        model=model,
    ...
    save_artifact(model, paths["models"] / "catboost_model.joblib")
    model.save_model(str(paths["models"] / "catboost_model.cbm"))
    ...
    print(f"CatBoost training complete. Outputs saved to: {output_dir.resolve()}")


def build_catboost_model(args: argparse.Namespace, num_classes: int):
    try:
        from catboost import CatBoostClassifier
    except ImportError as exc:
        raise ImportError("catboost is required. Install it with: python -m pip install catboost") from exc
...
def fit_catboost(*, model, X_train, y_train, X_val, y_val, sample_weight, verbose: int):
    try:
        model.fit(
            X_train,
            y_train,
        ...
        if "cuda" in message or "gpu" in message:
            raise RuntimeError(
                "CatBoost failed in GPU mode. Submit the CPU sbatch or check that the GPU node has a compatible CUDA/CatBoost setup."
            ) from exc
        raise
...
def save_feature_importance(model, feature_names, paths: Dict[str, Path], top_n: int) -> None:
    ...
    plt.barh(plot_df["feature"], plot_df["importance"], color="#C05621")

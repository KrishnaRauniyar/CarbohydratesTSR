#!/usr/bin/env python
"""Train an SVM classifier for TSR key-frequency feature tables."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an SVM classifier for TSR features.")
    ...
    parser.add_argument(
        ...
        help="Feature scaler. SVM usually needs standard or robust scaling.",
    )
    ...
    parser.add_argument("--svm_type", default="linear_svc", choices=["linear_svc", "svc"], help="SVM implementation.")
    parser.add_argument("--C", type=float, default=1.0, help="Regularization strength.")
    parser.add_argument("--kernel", default="rbf", choices=["linear", "rbf", "poly", "sigmoid"], help="Kernel for --svm_type svc.")
    parser.add_argument("--gamma", default="scale", help="Kernel coefficient for SVC.")
    parser.add_argument("--degree", type=int, default=3, help="Polynomial degree for SVC.")
    ...
    parser.add_argument(
        "--calibrate",
        action="store_true",
        help="Calibrate SVM decision scores to probabilities. More accurate but slower.",
    )
    parser.add_argument("--calibration_cv", type=int, default=3, help="CV folds for calibration.")
    parser.add_argument(
        "--backend",
        default="sklearn",
        choices=["sklearn", "auto", "cuml"],
        help="Use sklearn, or try RAPIDS cuML when available for smaller SVC experiments.",
    )
...
def main() -> None:
    ...
    model, backend_used = build_svm(args)
    config["backend_used"] = backend_used
    config["probability_note"] = (
        "predict_proba was used when available; otherwise decision scores were softmax-normalized."
    )
    save_json(config, paths["root"] / "config.json")
    save_json(config, paths["config"] / "config.json")
    ...
    save_artifact(model, paths["models"] / "svm_model.joblib")
    ...
    print(f"SVM training complete. Outputs saved to: {output_dir.resolve()}")


def build_svm(args: argparse.Namespace):
    if args.backend in {"auto", "cuml"} and args.svm_type == "svc" and args.balance_method != "class_weight":
        try:
            from cuml.svm import SVC as CuMLSVC

            model = CuMLSVC(C=args.C, kernel=args.kernel, gamma=args.gamma, degree=args.degree, probability=True)
            return model, "cuml"
        except Exception as exc:
            if args.backend == "cuml":
                raise RuntimeError("Requested --backend cuml, but cuML SVC could not be imported.") from exc
            warnings.warn(f"cuML is unavailable; falling back to sklearn. Details: {exc}")
    elif args.backend in {"auto", "cuml"}:
        warnings.warn("GPU/cuML SVM path is only attempted for --svm_type svc without class_weight; using sklearn.")

    class_weight = "balanced" if args.balance_method == "class_weight" else None
    if args.svm_type == "linear_svc":
        ...
        base = SVC(
            ...
            degree=args.degree,
            class_weight=class_weight,
            probability=not args.calibrate,
            max_iter=args.max_iter,
            tol=args.tol,
            random_state=args.random_state,
            verbose=True,
    ...
    if args.calibrate:
        try:
            return CalibratedClassifierCV(estimator=base, cv=args.calibration_cv), "sklearn_calibrated"
        except TypeError:
            return CalibratedClassifierCV(base_estimator=base, cv=args.calibration_cv), "sklearn_calibrated"
    return base, "sklearn"


def evaluate_split(*, model, X, y, proteins, class_names, split_name: str, paths: Dict[str, Path]):
    y_prob = predict_probability_like(model, X)
...
def predict_probability_like(model, X: pd.DataFrame) -> np.ndarray:
...
def save_coefficients(model, feature_names, class_names, paths: Dict[str, Path], top_n: int) -> None:
    ...
    coefficient_df.insert(0, "class_or_hyperplane", class_names[: coef.shape[0]])
    ...
    plt.barh(plot_df["feature"], plot_df["mean_abs_coefficient"], color="#718096")
    plt.title("Top SVM Coefficients")
